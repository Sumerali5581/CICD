...
<head>
  ...
  <title>Add to your course</title>
  <style>
    *{
      margin: 0;
      padding: 0;
...
//  String user=(String)  request.getAttribute("user");
  ...
  Connection con2 = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe", "hr", "hr");
  String s="select f_name, l_name, prn, username from students";
  PreparedStatement stmt1 = con2.prepareStatement(s);
  ResultSet rs13 = stmt1.executeQuery();
%>
  ...
  <main class="table">
    ...
    <section class="table_body">
      <table>
        <thead>
          <tr>
            <th style="padding-left: 110px;">Name of the Student</th>
            <th> PRN number</th>
            <th></th>
          </tr>
        </thead>
        <tbody>
          <%while(rs13.next()) {%> 
          <tr>
            <td style="padding-left: 100px;"><%=rs13.getString(1)%> <%=rs13.getString(2)%></td>
            <td><%=rs13.getString(3)%></td>
            <!--changes-->
            <td>
              <form method="post" action="done_add_new_student_byTeach">
                <input type="hidden" name="stud" value="<%= rs13.getString(4) %>">
                <button class="btn" type="submit"
                  onclick="return confirm('Are you sure you want to add this student to your course?')">Add to Course</button>
              </form>
            </td>
            <!--upto here-->
          </tr>
...
<%
        //closing connection
    try { rs13.close(); } catch (Exception e) { /* Ignored */ }
    try { stmt1.close(); } catch (Exception e) { /* Ignored */ }
    try { con2.close(); } catch (Exception e) { /* Ignored */}
